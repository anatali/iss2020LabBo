...
class Maitrewalker ( name: String, scope: CoroutineScope  ) : ActorBasicFsm( name, scope ){
		...
		return { //this:ActionBasciFsm
				state("s0") { //this:State
					action { //it:State
						...
						println("&&&  maitrewalker STARTS")
					}
					 transition( edgeName="goto",targetState="waitCmd", cond=doswitch() )
				}	 
				state("waitCmd") { //this:State
					action { //it:State
						println("&&&  maitrewalker waits for a command 'movetoCell'")
					}
				...
				state("walk") { //this:State
					action { //it:State
						...
						if( checkMsgContent( Term.createTerm("movetoCell(X,Y)"), Term.createTerm("movetoCell(X,Y)"), 
								 ...
								 XT = payloadArg(0)
											   YT = payloadArg(1)			  
								println("&&&  maitrewalker  | MOVING to ($XT,$YT)")
				...
				state("noPlan") { //this:State
					action { //it:State
						println("&&&  maitrewalker | NO PLAN FOUND for MOVING to ($XT,$YT)")
				...
				state("stepFailed") { //this:State
					action { //it:State
						 obstacleFound = true  
						println("maitrewalker | stepFailed")
						if( checkMsgContent( Term.createTerm("stepfail(DURATION,CAUSE)"), Term.createTerm("stepfail(DURATION,CAUSE)"), 
								...
								println("maitrewalker stepFail D= $D, BackTime = ${BackTime}")
								if(  D > BackTime  
				...
				state("sendSuccessAnswer") { //this:State
					action { //it:State
						println("&&&  maitrewalker POINT ($XT,$YT) REACHED")
