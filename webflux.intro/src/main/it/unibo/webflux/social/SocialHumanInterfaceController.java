...
public class SocialHumanInterfaceController {		//Book pg. 55
	...
	public Mono<String> index(Model model) {	//Book pg. 58
		try {
 			fluxOfImages = imageService.findAllImages();
 			model.addAttribute("images", fluxOfImages );		//populate asynch
		}catch(Exception e) {
			System.out.println("SocialHumanInterfaceController | ERROR" + e);
		}
	...
	//curl localhost:8082/images 	NOT ALLOWED
	// tag::3[]
	...
	public Mono<String> deleteFile(@PathVariable String filename) {		//Book pg. 58
		return imageService.deleteImage(filename)
			 .then(Mono.just("redirect:/"));
//				.then( Mono.just(FILENAME +  " has been deleted") );
//		return Mono.just(FILENAME +  " must be deleted");
	}
	// end::4[]
