...
@kotlinx.coroutines.ExperimentalCoroutinesApi
     override fun update(o: Observable?, arg: Any?) {
 	        GlobalScope.launch{
				...
				val event = MsgUtil.buildEvent("buttonEventEmitter","buttonCmd", "buttonCmd($name)")
				println("buttonEventEmitter click => emits buttonCmd")
				a.emit(event, avatar=true)
				//a.emit( event )	//The event is not propagates to the emitter (myself)		 	
	        }
    }
}
